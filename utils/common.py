...
def merge_config():
    ...
    items = [
        'dataset', 'data_root', 'epoch', 'batch_size', 'optimizer', 'learning_rate', 'weight_decay', 'momentum',
        'scheduler', 'steps', 'gamma', 'warmup', 'warmup_iters', 'use_aux', 'griding_num', 'backbone', 'sim_loss_w',
        'shp_loss_w', 'note', 'log_path', 'finetune', 'resume', 'test_model', 'test_work_dir', 'num_lanes'
    ]
    for item in items:
...
def save_model(net, optimizer, epoch, save_path, distributed):
    if is_main_process():
        ...
        torch.save(state, os.path.join(save_path, 'ufld-%03d.pt' % epoch))
        torch.save(state, os.path.join(save_path, 'ufld-final.pt'))
