...
class LaneClsDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        for i in range(label_np.shape[0]):
            for j in range(label_np.shape[1]):
                print(label_np[i][j],end=' ')
            print()

        print(label_np.shape)
        label_np_list=list(label_np.flatten())
        label_np_list=[i for i in label_np_list if i!=0]
        # print(label_np_list)
        num_map={}
        for i in label_np_list:
    ...
    def _get_index(self, label):
        ...
        # See issue : https://github.com/cfzd/Ultra-Fast-Lane-Detection/issues/319#issuecomment-1336869864
