...
def get_train_loader(batch_size, data_root, griding_num, dataset, use_aux, distributed, num_lanes):
    ...
    img_transform = transforms.Compose(
        [
            ...
            transforms.ColorJitter(# https://pytorch.org/vision/main/generated/torchvision.transforms.ColorJitter.html
                brightness=0.02,
                contrast=0.5,
                hue=0.5,
            ),
...
def get_test_loader(batch_size, data_root, dataset, distributed):
    ...
    if dataset == 'CULane':
        test_dataset = LaneTestDataset(
            data_root, os.path.join(data_root, 'list/test.txt'), img_transform=img_transforms
        )
        cls_num_per_lane = 18
    elif dataset == 'Tusimple':
